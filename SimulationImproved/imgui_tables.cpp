...
bool    ImGui::BeginTableEx(const char* name, ImGuiID id, int columns_count, ImGuiTableFlags flags, const ImVec2& outer_size, float inner_width)
{
    ...
    if (instance_no > 0)
    {
        ...
        instance_id = GetIDWithSeed(instance_no, GetIDWithSeed("##Instances", NULL, id)); // Push "##Instances" followed by (int)instance_no in ID stack. // parasoft-suppress  CODSTA-MCPP-04 "External Library"
    }
    else
    {
        instance_id = id;
    ...
    if (use_child_window)
    {
        ...
        if (override_content_size.x != FLT_MAX || override_content_size.y != FLT_MAX)
            SetNextWindowContentSize(ImVec2(override_content_size.x != FLT_MAX ? override_content_size.x : 0.0f, override_content_size.y != FLT_MAX ? override_content_size.y : 0.0f)); // parasoft-suppress  MISRA2004-13_3 "External Library" // parasoft-suppress  CODSTA-225 "External Library"
        ...
        IM_ASSERT(table->InnerWindow->WindowPadding.x == 0.0f && table->InnerWindow->WindowPadding.y == 0.0f && table->InnerWindow->WindowBorderSize == 0.0f); // parasoft-suppress  MISRA2004-13_3 "External Library"
    ...
    const bool pad_inner_x = (flags & ImGuiTableFlags_NoPadInnerX) ? false : true; // parasoft-suppress  CODSTA-CPP-65 "External Library"
    ...
    const float outer_padding_for_border = (flags & ImGuiTableFlags_BordersOuterV) ? TABLE_BORDER_SIZE : 0.0f; // parasoft-suppress  CODSTA-CPP-65 "External Library"
    ...
    if (old_columns_count != 0 && old_columns_count != columns_count)
    {
        ...
        table->RawData = NULL; // parasoft-suppress  CODSTA-MCPP-04 "External Library"
    }
...
void ImGui::TableUpdateLayout(ImGuiTable* table)
{
    ...
    if (width_remaining_for_stretched_columns >= 1.0f && !(table->Flags & ImGuiTableFlags_PreciseWidths))
        for (int order_n = table->ColumnsCount - 1; stretch_sum_weights > 0.0f && width_remaining_for_stretched_columns >= 1.0f && order_n >= 0; order_n--) // parasoft-suppress  MISRA2004-13_4 "External Library"
        {
...
void    ImGui::EndTable()
{
    ...
    if (inner_window != outer_window)
    {
        ...
        g.CurrentTable = NULL; // To avoid error recovery recursing // parasoft-suppress  CODSTA-MCPP-04 "External Library"
        EndChild();
        g.CurrentTable = table;
    ...
    {
        const float decoration_size = (table->Flags & ImGuiTableFlags_ScrollX) ? inner_window->ScrollbarSizes.y : 0.0f; // parasoft-suppress  CODSTA-CPP-65 "External Library"
    ...
    g.CurrentTable = temp_data ? g.Tables.GetByIndex(temp_data->TableIndex) : NULL; // parasoft-suppress  CODSTA-CPP-65 "External Library"
    if (g.CurrentTable)
    {
...
void ImGui::TableSetupScrollFreeze(int columns, int rows)
{
    ...
    table->FreezeRowsCount = (table->InnerWindow->Scroll.y != 0.0f) ? table->FreezeRowsRequest : 0; // parasoft-suppress  MISRA2004-13_3 "External Library"
...
int ImGui::TableGetColumnCount()
{
    ImGuiContext& g = *GImGui;
    ImGuiTable* table = g.CurrentTable;
    return table ? table->ColumnsCount : 0; // parasoft-suppress  CODSTA-CPP-65 "External Library"
}
...
void ImGui::TableEndRow(ImGuiTable* table)
{
    ...
    // Logging
    if (g.LogEnabled)
        LogRenderedText(NULL, "|"); // parasoft-suppress  CODSTA-MCPP-04 "External Library"
...
void ImGui::TableSetColumnWidth(int column_n, float width)
{
    ...
    if (column_0->WidthGiven == column_0_width || column_0->WidthRequest == column_0_width) // parasoft-suppress  CODSTA-225 "External Library"
        return;
    ...
    if (column_1 == NULL) // parasoft-suppress  CODSTA-MCPP-04 "External Library"
        column_1 = (column_0->PrevEnabledColumn != -1) ? &table->Columns[column_0->PrevEnabledColumn] : NULL; // parasoft-suppress  CODSTA-MCPP-04 "External Library"
    if (column_1 == NULL) // parasoft-suppress  CODSTA-MCPP-04 "External Library"
        return;
...
void ImGui::TableSetupDrawChannels(ImGuiTable* table)
{
    ...
    const int channels_for_row = (table->Flags & ImGuiTableFlags_NoClip) ? 1 : table->ColumnsEnabledCount; // parasoft-suppress  CODSTA-CPP-65 "External Library"
...
void ImGui::TableAngledHeadersRowEx(ImGuiID row_id, float angle, float max_label_width, const ImGuiTableHeaderData* data, int data_count)
{
    ...
    IM_ASSERT(table != NULL && "Need to call TableHeadersRow() after BeginTable()!"); // parasoft-suppress  CODSTA-MCPP-04 "External Library"
    ...
    table->AngledHeadersSlope = (sin_a != 0.0f) ? (cos_a / sin_a) : 0.0f; // parasoft-suppress  MISRA2004-13_3 "External Library"
    ...
    ButtonBehavior(row_r, row_id, NULL, NULL); // parasoft-suppress  CODSTA-MCPP-04 "External Library"
    KeepAliveID(row_id);
...
void ImGui::TableSaveSettings(ImGuiTable* table)
{
    ...
    if (settings == NULL) // parasoft-suppress  CODSTA-MCPP-04 "External Library"
    {
    ...
    {
        const float width_or_weight = (column->Flags & ImGuiTableColumnFlags_WidthStretch) ? column->StretchWeight : column->WidthRequest; // parasoft-suppress  CODSTA-CPP-65 "External Library"
        ...
        column_settings->IsStretch = (column->Flags & ImGuiTableColumnFlags_WidthStretch) ? 1 : 0; // parasoft-suppress  CODSTA-CPP-65 "External Library"
...
static void TableSettingsHandler_ReadLine(ImGuiContext*, ImGuiSettingsHandler*, void* entry, const char* line)
{
    ...
    if (sscanf(line, "RefScale=%f", &f) == 1) { settings->RefScale = f; return; } // parasoft-suppress  CODSTA-CPP-01 "External Library"

    if (sscanf(line, "Column %d%n", &column_n, &r) == 1) // parasoft-suppress  CODSTA-CPP-01 "External Library"
    {
        ...
        if (sscanf(line, "Visible=%d%n", &n, &r) == 1)          { line = ImStrSkipBlank(line + r); column->IsEnabled = (ImU8)n; settings->SaveFlags |= ImGuiTableFlags_Hideable; } // parasoft-suppress  CODSTA-CPP-01 "External Library"
...
void ImGui::DebugNodeTable(ImGuiTable* table)
{
    ...
    for (int n = 0; n < table->ColumnsCount; n++)
    {
        ...
        ImFormatString(buf, IM_ARRAYSIZE(buf),
            ...
            (column->Flags & ImGuiTableColumnFlags_WidthFixed) ? "WidthFixed " : "", // parasoft-suppress  CODSTA-CPP-65 "External Library"
